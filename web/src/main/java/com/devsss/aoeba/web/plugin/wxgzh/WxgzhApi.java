...
public class WxgzhApi {
    ...
    @Value("${aoeba.plugin.storage.name}")
    ...
    public WxgzhApi(WxgzhConfig wxgzhConfig) {
        this.wxgzhConfig = wxgzhConfig;
        this.access_token_url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=" + wxgzhConfig.getAppid()
                + "&secret=" + wxgzhConfig.getSecret();
    }
    ...
    public String uploadContentImg(String filename) {
        ...
        try {
            ...
            JSONObject jsonObject = getResponseJson(response, "上传图文消息内的图片到微信获取URL失败");
    ...
    public String html2WxDraft(String htmlContent) throws Exception {
        ...
        for (String imgUrl : imgUrls) {
            ...
            String url;
            if (imgUrl.startsWith(storageUrl) && "upyunStorage".equals(storageName)) {
                url = imgUrl + "!" + reqSecretKey;
            } else {
                url = imgUrl;
            }
            // 下载文件到本地
            ...
            log.debug("找到imgUrl:" + url + "准备替换为wxUrl:" + wxUrl);
     ...
     * @return 草稿的media_id
     */
    public String addDraft(String title, String author, String wxContent, String ywUrl, String mediaId) throws Exception {
...
//        content.put("digest",digest); 图文摘要
        ...
        content.put("need_open_comment", 0);
     ...
     * @throws Exception 异常
     */
    public String[] addMaterial(MaterialType materialType, String filePath, String... videoInfo) throws Exception {
        // https https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=ACCESS_TOKEN&type=TYPE
        String url = "https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=" + getToken() + "&type=";
        switch (materialType) {
            case image:
                url += "image";
                break;
...
//            }
            if (videoInfo.length == 2) {
                ...
                throw new Exception("materialType为video时,需传递videoInfo,长度为2的数组(title及introduction)");
            }
        }
    ...
    public boolean freePublishDraft(String draftId) throws Exception {
        ...
        String url = "https://api.weixin.qq.com/cgi-bin/freepublish/submit?access_token=" + getToken();
        JSONObject jsonObject = new JSONObject();
